...
class DatabaseGenUnit():
    ...
    def download_data(self, starttime='20160101', endtime='20160110',
                      countries=['SE', 'NO', 'DK', 'FI', 'LT', 'LV', 'EE']):
        ...
        for day in days:
            ...
            for country in countries:
                ...
                data = get_entsoe_gen_data(datatype=4, area=country, start=day.strftime('%Y%m%d'),
                        ...
                        if unit['id'] not in unit_list:
                            ...
                            c.execute(
                                "INSERT INTO units(id,name,country,type,resource) values ('{0}','{1}','{2}','{3}','{4}')".format(
                                    unit['id'], unit['name'], country, unit['production_type'],
...
class Database():
    ...
    def download_cap_per_type_data(self, start_year=2015, end_year=2018, areas=[]):
        ...
        for year in range(start_year, end_year):
            for area in area_codes:
                ...
                data = get_entsoe_gen_data(datatype=1, area=area, start='{0}0101'.format(year),
                                           end='{0}0101'.format(year))
    ...
    def get_entsoe_production_stats(self, starttime='20180101:00', endtime='20181231:23',
                                    thermal=['Biomass', 'Brown coal', 'Coal-gas', 'Gas', 'Hard coal', 'Oil',
                                             'Oil shale', 'Peat', 'Waste'], excelfile=None):
    ...
    def select_gen_per_type_wrap(self, starttime='20180101:00', endtime='20180107:23', areas=area_codes,
                                 thermal=['Biomass', 'Brown coal', 'Coal-gas', 'Gas', 'Hard coal', 'Oil', 'Oil shale',
                                          'Peat', 'Waste'], hydro=['Hydro ror', 'Hydro res', 'Hydro pump'],
    ...
    """
        ...
        pd_data = self.select_gen_per_type_data(areas=[a for a in areas if 'SE' not in a], \
                                                starttime=(str_to_date(starttime) + datetime.timedelta(
                                                    hours=-1)).strftime('%Y%m%d:%H'), \
                                                endtime=(str_to_date(endtime) + datetime.timedelta(hours=-1)).strftime(
                                                    '%Y%m%d:%H'))
...
def compare_nuclear_generation():
    ...
    # select nuclear
    se_nuclear = [idx for idx in plants.index if
                  plants.at[idx, 'type'] == 'Nuclear' and plants.at[idx, 'country'] == 'SE']
    fi_nuclear = [idx for idx in plants.index if
                  plants.at[idx, 'type'] == 'Nuclear' and plants.at[idx, 'country'] == 'FI']
